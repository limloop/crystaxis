...
use crate::sprite3d::systems::prepare_and_draw_sprites;
...
impl Plugin for Sprite3dRenderPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, setup_sprite3d_cache)
           ...
           .add_systems(PostUpdate, prepare_and_draw_sprites);
    }
}

/// Создаёт общий кэш меша и материала, а также камеру и освещение
fn setup_sprite3d_cache(
    ...
    // Единый меш для всех частиц
    let mesh = Mesh3d(meshes.add(quad_mesh(0.2, 0.2))); // размер спрайта в мировых единицах
